...
src/%.o: ../src/%.cpp
	...
	nvcc -G -g -O0 -gencode arch=compute_37,code=sm_37  -odir "src" -M -o "$(@:%.o=%.d)" "$<"
...
src/%.o: ../src/%.cu
	...
	@echo 'Invoking: NVCC Compiler'
	nvcc -G -g -O0 -gencode arch=compute_37,code=sm_37  -odir "src" -M -o "$(@:%.o=%.d)" "$<"
	nvcc -G -g -O0 --compile --relocatable-device-code=true -gencode arch=compute_37,code=compute_37 -gencode arch=compute_37,code=sm_37  -x cu -o  "$@" "$<"
	@echo 'Finished building: $<'
	@echo ' '
